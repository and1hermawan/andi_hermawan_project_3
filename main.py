...
def insert_raw_data():
  ...
  df.to_sql(name='andi_raw_covid', con=engine, if_exists="replace", index=False)
  engine.dispose()
...
def insert_raw_to_warehouse(schema):
    mysql_auth = MySQL(credential['mysql_lake'])
    engine, engine_conn = mysql_auth.connect()
    data = pd.read_sql(sql='andi_raw_covid', con=engine)
    engine.dispose()
    ...
    fact_district_yearly = insert_fact_district_daily(data, dim_case)
...
if __name__ == '__main__':
  insert_raw_data()
  create_star_schema(schema='digital_scola_de6')
  insert_raw_to_warehouse(schema='digital_scola_de6')